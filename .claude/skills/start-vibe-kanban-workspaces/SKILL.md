...
1. **Report file path** — Markdown report containing Vibe Kanban issue IDs (e.g., `docs/tasks/01-project-setup/01-project-setup-tasks-vibe-kanban-report.md`)
...
|---------------|------|-------|-------|-----------|
| `{uuid}` | T1 — Title | 1 | app/Models/Product.php | None |
```
...
| Parameter | Value |
|-----------|-------|
| `name` | Format: `{TaskID} — {Title}` (e.g., `"T1 — Create composer.json and bootstrap Laravel app"`) |
...
7. **Name format** — Use `{TaskID} — {Title}` matching report convention (e.g., `"T1 — Create composer.json"`).
