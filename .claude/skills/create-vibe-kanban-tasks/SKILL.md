...
| **Title** | Rest of heading after task ID | `Create composer.json and bootstrap Laravel app` |
| **Batch** | Section heading or summary table | `Batch 1: Project Scaffold` |
| **Files** | Under `**Files:**` or summary table | `modern/composer.json` |
...
| `title` | `{TaskID} — {Title}` (e.g., `"T1 — Create composer.json and bootstrap Laravel app"`) |
...
|---------------|------|-------|-------|-----------|
| `{issue_id}` | T1 — Title | 1 | app/Models/Product.php | None |
| `{issue_id}` | T2 — Title | 1 | app/Repositories/ProductRepository.php | None |
| ... | ... | ... | ... | ... |
...
Batch 4: DB Migration + Seed (depends on Batch 3)
  T10 ─→ T11
      │
Batch 5: Services + DTOs (depends on T2, T4)
  T12  T14  T15          ← parallel
      │
Batch 6: Controllers (depends on T12)
  T13
      │
